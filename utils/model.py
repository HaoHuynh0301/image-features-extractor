...
from keras.layers import Input, Dense, Dropout, LSTM, Embedding, concatenate, RepeatVector, TimeDistributed, Bidirectional, GlobalAveragePooling2D
...
def CNNModel(model_type):
	if model_type == 'inceptionv3':
		...
		model = create_model(num_classes=1001, dropout_prob=0.2, weights=None, include_top=False); model.load_weights('/content/drive/MyDrive/HAO_LV/inception-v4_weights_tf_dim_ordering_tf_kernels_notop.h5')
	x = model.output
	x = GlobalAveragePooling2D(data_format='channels_last')(x); feature_model = Model(model.input, x, name='feature_inception_v4')
